...
class GANModule(LightningModule):
    def __init__(self, gen, disc, latent_dim, lr, num_disc_steps, loss_type):
        super().__init__()
        self.gen = gen
        self.disc = disc
...
>>>>>>> refs/heads/main
    ...
    def training_step(self, batch, batch_idx):
        ...
        self.log_dict(
            {"train_gen_loss": g_loss, "train_disc_loss": d_loss}, prog_bar=True
        )
    ...
    def sample(self, cond, seq_len, n_samples):
        cond = (
            torch.FloatTensor(cond)[None, ...].repeat(n_samples, 1, 1).to(self.device)
        )
